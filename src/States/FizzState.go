...
type GameState1 struct {
	model_uniform int32
	texture, shader uint32
	model *Model.Model
	mesh *fizzle.Renderable
...
func (game *GameState1) Init(manager *StateManager, width int, height int, window *glfw.Window) State {
	...
	game.texture, temp_err = Shader.NewTexture("res/cat/cat.png")
	if temp_err != nil {
		panic(temp_err)
	}
	...
	game.model = new(Model.Model).Init("res/new_cat/cat.obj", game.shader)

	game.mesh = fizzle.NewRenderable()
...
func (game *GameState1) Update(elapsed float32) {
...
func (game *GameState1) Stop() bool {
	return true
}
